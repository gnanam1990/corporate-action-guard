...
| Lifecycle state machine                              | IMPLEMENTED          | `packages/domain/src/b20/lifecycle.ts` (facts) and `packages/reconciler/src/b20-state-machine.ts` (operational status). The X Layer machine is untouched.                      |
...
| **Raw / share-equivalent / value as distinct types** | IMPLEMENTED          | `packages/domain/src/b20/quantities.ts`. ADR 0006.                                                                                                                             |
| **Price basis as part of a price's type**            | IMPLEMENTED          | `PricePoint<Basis>` carries basis, decimals, feed, round and freshness. A bare price cannot enter valuation.                                                                   |
| **Double-multiplier rejection**                      | IMPLEMENTED          | Fails to typecheck, and `rejectDoubleMultiplier` refuses it at runtime boundaries before any arithmetic.                                                                       |
