...
behaviour of every boundary. It describes the *agreed target architecture*; it is not
...
| API → receipt signer | 5s | retry only by idempotency key; timeout is an *unknown* outcome, never an implicit success | Signer re-loads and re-verifies the ALLOW decision and its evidence transactionally before signing | **Fail closed.** Never emits a placeholder signature. |
